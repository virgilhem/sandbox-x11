...
int main(int argc, char *argv[])
{
    ...
    /* DENY_RULE (kcmp); required by firefox (pdf.js) */
    ...
    /* adding rules to block TIOCSTI and TIOCLINUX ioctls */
    if (seccomp_rule_add(ctx, SCMP_ACT_KILL_PROCESS, SCMP_SYS(ioctl), 1,
					SCMP_A1(SCMP_CMP_MASKED_EQ, 0xFFFFFFFFu, TIOCSTI)) <0)
      goto out;
    if (seccomp_rule_add(ctx, SCMP_ACT_KILL_PROCESS, SCMP_SYS(ioctl), 1,
					SCMP_A1(SCMP_CMP_MASKED_EQ, 0xFFFFFFFFu, TIOCLINUX)) <0)
      goto out;
